...
public static class FreeSqlExtend
{
    ...
    public static void RegisterFreeSql(this IServiceCollection services, IConfiguration configuration)
    {
        ...
        {
                { DbEnum.safiDb, configuration.GetSection("DbConnectionStrings:FPLConnectionString").Get<DbConnectionOptionsConfig>() }
            };
        var fsql = new FreeSqlCloud();
        fsql.DistributeTrace = log => Console.WriteLine(log.Split('\n')[0].Trim());

        fsql.Register(DbEnum.safiDb, () => new Lazy<IFreeSql>(() =>
        {
            DbConnectionOptionsConfig oFreeSqlDbConnectionItemConfig = dicDbConnections[DbEnum.safiDb];
